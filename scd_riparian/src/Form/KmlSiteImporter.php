...
/**
 * Provides a form for importing folders of KML placemarks as sites.
 */
class KmlSiteImporter extends FormBase {
...
//    $geometries = $this->serializer->deserialize($data, GeometryWrapper::class, 'geometry_kml');
  ...
  protected function loadSiteFromFolder(SimpleXmlElement $folder): array {
    $data = [];
    if (isset($folder->Placemark)) {
      $site_name = $folder->name;
    }
    else if (isset($root->Folder)) {
      foreach ($root->Folder as $folder) {
        array_merge($data, $this->loadSiteFromFolder($folder));
      }
    }
    return $data;
  }
